...
function sfMultiDriver()
  ...
  langNames = containers.Map(...
      {1,2,3,4,5,6,7}, ...
      {'IL10-Set1-LR','bengali', 'english', 'indonesian', 'tagalog', 'thai', 'zulu'});
...
% 	  nonNanMean(pranucs4));
  ...
  %fprintf('average performance per language, using meta, ditto+pfmeans, ditto+pfstds, 80-20\n');
   fprintf('average performance per language, using meta + pfmeans + pfstds\n');
...
%   end
  for lang = 1:nlanguages
    fprintf('%13s %.3f\n', ...
	    langNames(lang),  nonNanMean(pranucs3(lang, :)));
  end
...
%   end
  for lang = 1:nlanguages
    fprintf('%13s %.2f \n', langNames(lang), pranucs3(lang, 6));
  end
%   fprintf('     AVERAGES  %.3f  %.3f  %.3f  %.3f\n', mean(pranucs1(:, 6)), ...
% 	  mean(pranucs2(:, 6)), mean(pranucs3(lang, 6)), nonNanMean(pranucs4(:,6)));

fprintf('\nPerformance on predicting Type\n');
for lang = 1:nlanguages
    fprintf('%13s %.2f \n', langNames(lang), pranucs3(lang, 5));
end
fprintf('\nPerformance on predicting Urgency\n');
for lang = 1:nlanguages
    fprintf('%13s %.2f \n', langNames(lang), pranucs3(lang, 3));
end
end
